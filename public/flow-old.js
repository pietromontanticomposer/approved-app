...
async function selectProject(projectId, projectName) {
  ...
  try {
    ...
    if (rightColumnEl) rightColumnEl.style.display = 'flex';
...
async function loadCues(projectId) {
  try {
    ...
    console.log('[loadCues] API response:', data);
    window.__state.cues = data.cues || [];
    console.log('[loadCues] Loaded', window.__state.cues.length, 'cues', window.__state.cues);
    await renderCueList();
  } catch (err) {
...
async function renderCueList() {
  ...
  if (!cueListEl) {
    console.warn('[renderCueList] DOM not ready, retrying in 50ms...');
    return new Promise(resolve => {
      setTimeout(() => renderCueList().then(resolve), 50);
    });
  }
  ...
  if (cues.length === 0) {
    ...
    console.log('[renderCueList] ✅ Rendered (empty)');
    return;
  }
...
async function renderPlayer(version) {
  ...
  if (!playerMediaEl || !playerTitleEl || !playPauseBtnEl) {
    console.warn('[renderPlayer] DOM not ready, retrying in 50ms...');
    return new Promise(resolve => {
      setTimeout(() => renderPlayer(version).then(resolve), 50);
    });
  }
...
async function renderComments() {
  ...
  if (!commentsListEl || !commentsSummaryEl) {
    console.warn('[renderComments] DOM not ready, retrying in 50ms...');
    return new Promise(resolve => {
      setTimeout(() => renderComments().then(resolve), 50);
    });
  }
  ...
  if (comments.length === 0) {
    ...
    console.log('[renderComments] ✅ Rendered (empty)');
    return;
  }
...
async function renderReferences() {
  ...
  if (!refsListEl) {
    console.warn('[renderReferences] DOM not ready, retrying in 50ms...');
    return new Promise(resolve => {
      setTimeout(() => renderReferences().then(resolve), 50);
    });
  }
  ...
  if (refs.length === 0) {
    ...
    console.log('[renderReferences] ✅ Rendered (empty)');
    return;
  }
